...
import { PERCENTAGE_CALCULATOR_META } from '../meta.js'
import { PercentageCalculatorInputSchema, getI18nKey } from '../validate.js'
...
const SLUG = PERCENTAGE_CALCULATOR_META.slug
...
pluginRegistry.register({
  ...
  version: '1.0.0',
  instrumentSlug: SLUG,
  compute: calculatePercentageCalculator,
  verify: (input, expected) => {
    const result = calculatePercentageCalculator(input)
...
pluginRegistry.register({
  ...
  version: '1.0.0',
  instrumentSlug: SLUG,
  schema: PercentageCalculatorInputSchema,
  validate: (raw) => {
    const result = PercentageCalculatorInputSchema.safeParse(raw)
