      ...
      <div className="flex-1 min-w-0">
        ...
        {milestone.is_completed && milestone.actual_value !== null && (
          <div className="text-xs text-slate-500">
            Actual: {milestone.actual_value} {milestone.actual_value !== milestone.target_value && `(target: ${milestone.target_value})`}
          </div>
        )}
      </div>
      <div className="text-xs text-slate-500 shrink-0">Wk {milestone.week}</div>
    </div>
  )
}
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-5">
        <div className="flex items-start justify-between gap-4">
          <div>
            <div className="text-xs text-blue-600 dark:text-blue-400 font-semibold uppercase tracking-widest mb-1">
              {strategy?.selected_strategy_name ?? 'Adaptive Plan'} · Week {currentWeek}
            </div>
            <h2 className="text-lg font-bold text-slate-900 dark:text-white leading-tight">
              {plan.goal}
            </h2>
            <div className="text-sm text-slate-500 mt-0.5">
              {plan.duration_weeks} weeks · {plan.adaptation_count > 0 && `adapted ${plan.adaptation_count}×`}
            </div>
          </div>
        ...
        <div className="mt-2 flex justify-between text-xs text-slate-400">
          <span>Start: {plan.current_value} {plan.cluster === 'weight' ? 'kg' : ''}</span>
          <span>Goal: {plan.goal_value} {plan.cluster === 'weight' ? 'kg' : ''}</span>
        </div>
      </div>
        ...
        <div className="rounded-2xl border border-emerald-200 dark:border-emerald-800 bg-emerald-50 dark:bg-emerald-950 p-4 flex items-center justify-between gap-4">
          <div>
            ...
            <div className="text-xs text-emerald-600 dark:text-emerald-400 mt-0.5">
              Record your progress so your plan can adapt.
            </div>
          </div>
          <button
      ...
      {/* Milestones */}
      <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-5">
        <h3 className="font-semibold text-slate-900 dark:text-white mb-4">Milestones</h3>
      ...
      {plan.check_ins.length > 0 && (
        <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-5">
          <h3 className="font-semibold text-slate-900 dark:text-white mb-4">Check-in History</h3>
          <div className="space-y-2">
            {[...plan.check_ins].reverse().slice(0, 5).map(ci => (
              <div key={ci.check_in_id} className="flex items-center gap-3 text-sm">
                ...
                <span className="text-slate-600 dark:text-slate-400">Week {ci.week}</span>
                <span className="font-medium text-slate-900 dark:text-white">
                  {ci.actual_value} {plan.cluster === 'weight' ? 'kg' : ''}
                </span>
                <span className="text-slate-400 text-xs ml-auto">
                  {ci.deviation_percent}% deviation
                </span>
              </div>
            ))}
          </div>
