...
//
// The user's home screen. Reads IntentState for all active clusters,
// selects the primary one (highest phase), renders full journey context.
//
...
//  6. Coach Insight — one data-derived insight
//
...
// ── Active clusters to check (those with calculators mapped) ──────────────────
const KNOWN_CLUSTERS: IntentCluster[] = ['weight', 'sleep', 'finance']

// ── Phase ordering for primary cluster selection ──────────────────────────────
    ...
    <div className="flex flex-col items-center text-center py-16 space-y-4">
      ...
      <div className="space-y-2">
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400 max-w-xs">
          Use any calculator to begin. Your first result unlocks your personal journey.
        </p>
      </div>
      <a
...
export function JourneyDashboard({ lang }: Props) {
  ...
  const insight  = buildCoachInsight(intent)
  ...
  return (
    ...
    >
      {/* 1. Hero — greeting + phase */}
      <DashboardHero intent={intent} lang={lang} />

      {/* 2. Today's Action — single most important next step */}
      <DashboardNextAction
        phase={phase}
      ...
      {/* 5. Achievements */}
      <DashboardAchievements instruments={intent.completedInstruments} limit={5} />

      {/* 6. Coach Insight */}
      {insight && <DashboardCoachInsight insight={insight} />}

      {/* Other active clusters — quick links */}
      {KNOWN_CLUSTERS.filter(c => c !== primaryCluster && (intents[c]?.completedInstruments.length ?? 0) > 0).length > 0 && (
        <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 overflow-hidden">
          ...
          <ul className="divide-y divide-slate-100 dark:divide-slate-800">
            {KNOWN_CLUSTERS.filter(c => c !== primaryCluster).map(cluster => {
              const ci = intents[cluster]
              if (!ci || ci.completedInstruments.length === 0) return null
              return (
                <li key={cluster}>
                  <a
                  ...
                  >
                    <div>
                      ...
                      <p className="text-[11px] text-slate-500 dark:text-slate-400">
                        {ci.completedInstruments.length} steps · {ci.currentPhase}
                      </p>
                    </div>
